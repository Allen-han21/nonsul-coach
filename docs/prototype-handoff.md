...
- 2026학년도 인문 1교시·2교시 각 2문항과 2027학년도 인문 모의 2문항, 총 6문항이다.
...
| 단계 | 상태 | 완료 근거 / 남은 작업 |
| --- | --- | --- |
| 1. 평가 자료 확정 | done | [평가 패키지](evaluation-packages.md), [출처 데이터](evaluation/source.json). 원본 해시 및 여섯 문항의 텍스트·관련 페이지 이미지 대조 완료. |
...
- 사용자 제공 동일 가이드북 49~60쪽을 사용해 2교시 문제 1·2, 제시문 <가>~<라>, 유의사항 및 서버 전용 평가 자료를 추가했다. 자연계열 자료는 포함하지 않았다.
- 문제 1은 라부부 소비의 심리적 역설, 문제 2는 팝마트 판매전략을 두 정의관과 기업윤리에 연결하는 평가다. 화면은 2026 1교시 2문항·2교시 2문항·2027 모의 2문항, 총 6문항을 제공한다.
- 52쪽 문제 1의 중복 닫는 괄호와 제시문의 원문 표기는 보존했다. 답안 분량은 두 문항 모두 800~1,000자다.
- 상위 main 구현 커밋 `054bdaf`. 여섯 패키지를 대상으로 단위 테스트 22개, 타입·린트·빌드·공개 자료 경계 검사 통과. 브라우저 테스트 코드는 확장했지만 이번 수정에서는 실행하지 않았다.
- 기존 Sites 저장소의 배포 소스 SHA: `85bc2d2a3f9b565be7e960b64dc3c4fb0976ce98`. 상위 구현 커밋의 web 하위 트리와 Sites 소스 트리 해시가 `3d2b9c51d986d75e807add32df42faea1e76b4fb`로 일치함을 확인했다.
- 저장 버전 2: `appgprj_6aa7aaf663c08191a0bf3dce49d3142c~appgver_df85164bbca08191ab58992820f4550c`.
- 배포 `appgdep_6aa7b057160881918a256a051ae993a4` 성공. runtime revision 2와 기존 URL `https://nonsul-coach-sungshin.dongwookq.chatgpt.site`를 유지했다.
- 배포 전 소유자 1명·그룹 0개·외부 방문자 0명인 접근 정책을 재확인했다. 배포 후 비인증 HTTP 401, 인증된 홈·화면 코드 자산 200, 배포된 화면 코드와 검증 빌드의 해시 일치를 확인했다. 이는 브라우저 화면·조작 검증이 아닌 HTTP 및 자산 무결성 검사다.
- 새 2교시 ID의 합성 요청은 503/NOT_CONFIGURED와 no-store를 반환했다. 실제 AI 호출 0회, 제공자·모델·키·비저장 확인 설정은 변경하지 않았다.
- 상위 GitHub 저장소를 푸시하지 않았다. Sites 자격 증명은 단일 명령 환경에만 사용하고 원격 URL·Git 설정·소스에 저장하지 않았다.
