...
- 학생용 한 화면. 학생 정보·답안·원문 요청/응답·분석 이력을 저장하거나 로그로 남기지 않는다.
...
| 3. 입력 화면 | done | 네 문항·두 입력란·개인정보 안내·참고 글자 수. 실제 미리보기와 브라우저 검증 완료. |
...
- `node scripts/verify-build.mjs`: 공개 자산 15개, 공식 예시답안/키 설정의 브라우저 노출 없음, 서버 출처와 검토 데이터 내용 일치.
...
- 공식 PDF/예시답안은 클라이언트에 배포하지 않는다. `web/lib/server/official.json`은 서버 번들에만 존재한다.
...
`scripts/prepare-evaluation.mjs`에 사용자 제공 PDF 경로를 전달한다. 원본 SHA-256이 다르면 중지한다. `docs/evaluation/source.json`과 서버의 `official.json`을 동기화할 때 기존 검토와 원본 해시를 먼저 확인한다.
